...
$users_per_page = 16; // Updated to 16 users per page
...
$sql = "SELECT id, gmail, name FROM user LIMIT $users_per_page OFFSET $offset"; 
...
<main>
    <div class="user-grid">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<p><strong>ID User: </strong>' . $row["id"] . '</p>';
                echo '<p><strong>Username: </strong>' . $row["name"] . '</p>';
                echo '<p><strong>Email: </strong>' . $row["gmail"] . '</p>';
                echo '</div>';
                echo '<a href="user_detail.php?id=' . $row["id"] . '" class="detail-btn">Detail</a>';
                echo '</div>';
            }
        } else {
            echo "No users found.";
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a href="user_manage.php?page=<?php echo $page - 1; ?>" class="pagination-button">Previous</a>
        <?php endif; ?>
        <span class="page-number"><?php echo $page; ?></span>
        <?php if ($page * $users_per_page < $total_users): ?>
            <a href="user_manage.php?page=<?php echo $page + 1; ?>" class="pagination-button">Next</a>
        <?php endif; ?>
    </div>
</main>
